...
def two_nearest_opponents_(X_):
    ...
    two_opponent_col = ["dis_first_op","dis_second_op"]
        ...
        for j in range(22):
            ...
            for k in range(22):
                if X_.iloc[i*22+k]["same_team"] == 0:
                    dis = np.sqrt((receiver_x - X_.iloc[i*22+k]["x_j"])**2 +(receiver_y - X_.iloc[i*22+k]["y_j"])**2)
                    if dis <= second_opponent_dis and dis > 0.01:
                        second_opponent_dis = dis
                        if dis <= first_opponent_dis:
...
def two_nearest_allies_(X_):
    n = X_.shape[0]
    first_ally_dis = 0
    second_ally_dis = 0
    two_allies_col = ["dis_first_al","dis_second_al"]
    X_additional_col = pd.DataFrame(data = np.zeros((n,len(two_allies_col))) ,columns = two_allies_col)
    
    
    #Filling the additional column with the two nearest allies
    receiver_x = 0
    receiver_y = 0
        ...
        for j in range(22):
            ...
            for k in range(22):
                if X_.iloc[i*22+k]["same_team"] == 1:
                    dis = np.sqrt((receiver_x - X_.iloc[i*22+k]["x_j"])**2 +(receiver_y - X_.iloc[i*22+k]["y_j"])**2)
                    if dis <= second_ally_dis and dis > 0.01:
                        second_ally_dis = dis
                        if dis <= first_ally_dis:
...
if __name__ == '__main__':
    ...
    """
    !!!!Use this to create X_features2.csv!!!
   
    X_LS_pairs_neighbors = two_nearest_opponents_(X_LS_pairs)
    X_LS_pairs_neighbors_2 = two_nearest_allies_(X_LS_pairs_neighbors)
    
    X_features = X_LS_pairs_neighbors_2[["distance", "same_team","dis_first_op","dis_second_op","dis_first_al","dis_second_al"]]
    """
    ...
    """
    X_TS_pairs_neighbors = two_nearest_opponents_(X_TS_pairs)
    X_TS_pairs_neighbors_2 = two_nearest_allies_(X_TS_pairs_neighbors)
    
    X_TS_features = X_TS_pairs_neighbors_2[["distance", "same_team","dis_first_op","dis_second_op","dis_first_al","dis_second_al"]]
    """
    
    X_TS_features = load_from_csv(prefix + "X_TS2.csv")
    # Predict
    ...
    predicted_score = cross_val_score(model,X_features,np.ravel(y_LS_pairs), cv = 5,scoring = 'f1').mean()
